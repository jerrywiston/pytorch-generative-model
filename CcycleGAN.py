...
class GeneratorZ(nn.Module):
    def __init__(self, z_dim, c_dim):
        ...
        self.fc5_z = nn.Linear(4*4*1024, z_dim)
        self.fc5_c = nn.Linear(4*4*1024, c_dim)
    ...
    def forward(self, x):
        ...
        # (1024, 4, 4)
        self.z_code = self.fc5_z(self.h_conv4.view(-1,1024*4*4))
        self.c_code = self.fc5_c(self.h_conv4.view(-1,1024*4*4))
        return self.z_code, self.c_code

class GeneratorX(nn.Module):
    def __init__(self, z_dim, c_dim):
        super(GeneratorX, self).__init__()
        self.fc1 = nn.Linear(z_dim+c_dim, 8*8*1024)
    ...
    def forward(self, z, c):
        z_c = torch.cat([z,c], 1)
        self.h_conv1 = F.relu(self.fc1(z_c).view(-1,1024,8,8)) 
        # (1024, 8, 8)
...
class DiscriminatorZ(nn.Module):
    def __init__(self, z_dim):
        ...
        self.fc1 = nn.Linear(z_dim+c_dim, 512)
...
netGz = GeneratorZ(z_dim, c_dim).to(device)
netGz.apply(weights_init)
netGx = GeneratorX(z_dim, c_dim).to(device)
...
c_fixed = torch.randn(64, c_dim, device=device)
...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        c_real = torch.randn(b_size, c_dim, device=device)
        ...
        # D(Z)
        dz_real, _ = netDz(z_real, c_real)
        dz_real_loss = nn.BCELoss()(dz_real, ones)
        z_gen, c_gen = netGz(x_real)
        dz_fake, _ = netDz(z_gen, c_gen)
        ...
        # Reconstruction Loss
        z_gen, c_gen = netGz(x_real)
        x_rec = netGx(z_gen, c_gen)
        rx_loss = nn.MSELoss()(x_rec, x_real)
        # Distribution Loss
        dz_fake, _ = netDz(z_gen, c_gen)
        gz_loss = nn.BCELoss()(dz_fake, ones)
        # Backward
        xzx_loss = rx_loss + lamda_z*gz_loss
        xzx_loss.backward()

        # (Z -> X -> Z)
        # Reconstruction Loss
        x_gen = netGx(z_real, c_real)
        z_rec, c_rec = netGz(x_gen)
        rz_loss = nn.MSELoss()(z_rec, z_real)
        # Distribution Loss
        dx_fake, _ = netDx(x_gen)
        gx_loss = nn.BCELoss()(dx_fake, ones)
        # Backward
        zxz_loss = rz_loss + lamda_x*gx_loss
        zxz_loss.backward()
        ...
        if i%200 == 0:
            # Output Images
            x_fixed = netGx(z_fixed, c_fixed).cpu().detach()
