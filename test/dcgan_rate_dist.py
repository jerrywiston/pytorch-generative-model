...
def distance_lossX(x1, x2, x3):
    dist12 = ((x1 - x2)**2).sum([1,2,3])
    dist23 = ((x2 - x3)**2).sum([1,2,3])
    dist13 = ((x1 - x3)**2).sum([1,2,3])
    dist_loss1 = torch.clamp(dist13 - dist12 - dist23, min=0)
    dist_loss2 = torch.clamp(dist12 - dist23 - dist13, min=0)
    dist_loss3 = torch.clamp(dist23 - dist12 - dist13, min=0)
    return (dist_loss1 + dist_loss2 + dist_loss3).sum()

def distance_loss(z1, z2, z3, x1, x2, x3):
    dz12 = ((z1 - z2)**2).mean([1])
    dz23 = ((z2 - z3)**2).mean([1])
    dz13 = ((z1 - z3)**2).mean([1])
    dx12 = ((x1 - x2)**2).mean([1,2,3])
    dx23 = ((x2 - x3)**2).mean([1,2,3])
    dx13 = ((x1 - x3)**2).mean([1,2,3])

    dist_loss1 = nn.MSELoss()(dz12*dx23, dx12*dz23)
    dist_loss2 = nn.MSELoss()(dz12*dx13, dx12*dz13)
    dist_loss3 = nn.MSELoss()(dz13*dx23, dx13*dz23)
    return (dist_loss1 + dist_loss2 + dist_loss3).mean()
...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        z3_samp = torch.randn(b_size, z_dim, device=device)
        x3_samp = netG(z3_samp)
        dist_loss = distance_loss(z1_samp, z2_samp, z3_samp, x1_samp, x2_samp, x3_samp)
        dist_loss.backward()
        optG.step()
