...
class Generator(nn.Module):
    def __init__(self, z_dim, ndf=64):
        ...
        self.ndf = ndf
        # (8,8,512) -> (8,8,512)
        self.fc1 = nn.utils.spectral_norm(nn.Linear(z_dim, 8*8*ndf*8))
        self.res1 = ResBlock(ndf*8,ndf*8)
        # (8,8,512) -> (16,16,256)
        self.up2 = nn.Upsample(scale_factor=2, mode='bilinear')
        self.res2 = ResBlock(ndf*8,ndf*4)
        # (16,16,256) -> (32,32,256)
        self.up3 = nn.Upsample(scale_factor=2, mode='bilinear')
        self.res3 = ResBlock(ndf*4,ndf*4)
        # (32,32,256) -> (64,64,128)
        self.up4 = nn.Upsample(scale_factor=2, mode='bilinear')
        self.res4 = ResBlock(ndf*4,ndf*2)
...
class DiscriminatorZ(nn.Module):
    def __init__(self, z_dim, ndf=512):
        ...
        self.fc1 = nn.utils.spectral_norm(nn.Linear(z_dim, ndf))
        self.fc2 = nn.utils.spectral_norm(nn.Linear(ndf, ndf))
        self.fc3 = nn.utils.spectral_norm(nn.Linear(ndf, ndf))
        self.fc4 = nn.utils.spectral_norm(nn.Linear(ndf, 1))
